...
func (s *WasmTestSuite) Swap(contractPath string, executeFunc func(contract sdk.AccAddress, sender sdk.AccAddress, msg bindings.TerraMsg, funds sdk.Coin) error) {
	...
	err := executeFunc(contractAddr, actor, msg, sdk.NewCoin(core.MicroLunaDenom, sdkmath.NewInt(1000)))
	...
	s.Require().Equal(actorBeforeSwap.AmountOf(core.MicroLunaDenom).Sub(sdkmath.NewInt(1000)), actorAfterSwap.AmountOf(core.MicroLunaDenom))
